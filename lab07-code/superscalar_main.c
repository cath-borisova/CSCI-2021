...
// Code to help us time execution
typedef struct timespec timespec_t;

double get_elapsed_time_sec(const timespec_t *start, const timespec_t *end) {
    unsigned long start_nanos = (int)1e9 * start->tv_sec + start->tv_nsec;
    unsigned long end_nanos = (int)1e9 * end->tv_sec + end->tv_nsec;
...
int main(int argc, char *argv[]) {
    ...
    printf("%s for %lu * 2^{%lu} = %lu iterations...\n", alg_name, mult,exp, iters);
    fflush(stdout);

    timespec_t begin_time;
    clock_gettime(CLOCK_REALTIME, &begin_time); // TODO Fill in function arguments
    alg_func(iters, &start, &delta);         // run the specified algorithm
    timespec_t end_time;
    clock_gettime(CLOCK_REALTIME, &end_time); // TODO Fill in function arguments
    double elapsed_sec = end_time.tv_sec - begin_time.tv_sec; // TODO Compute elapsed seconds between timestamps
